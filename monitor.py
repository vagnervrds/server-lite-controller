...
def calculate_usage_rates():
    ...
    prev = db.get_monitor_state('previous_values', {
        'disk_total': 0, 'network_total': 0, 'timestamp': 0
    })

    if prev['timestamp'] == 0:
        db.set_monitor_state('previous_values', {
...
def update_usage(disk_rate, network_rate):
    ...
    max_disk, max_net = db.update_max_if_greater(
        ...
        config['min_disk_rate'], config['min_network_rate']
    )

    disk_percent = (disk_rate / max_disk  * 100) if max_disk  > 0 else 0
    net_percent  = (network_rate / max_net * 100) if max_net   > 0 else 0

    below = (disk_percent  < config['disk_threshold'] and
             net_percent   < config['network_threshold'])
...
def check_usage_thresholds():
    ...
    below    = (disk_pct  < config['disk_threshold'] and
                net_pct   < config['network_threshold'])

    if below:
        if not idle_timer_active:
            idle_timer_active = True
            last_activity_time['start_time'] = now
            logger.info(f"Inatividade detectada. Timer de {config['idle_time_threshold']} min iniciado.")
        else:
                ...
                with open('/proc/diskstats', 'r') as f:
                    for line in f:
                        ...
                        if dev.startswith(('sd', 'hd', 'vd', 'nvme', 'mmcblk')):
                            r = int(parts[5]) * 512
                            w = int(parts[9]) * 512
                            info[dev] = {'read_bytes': r, 'write_bytes': w, 'total_bytes': r + w}
            except Exception:
                pass
            try:
                for part in psutil.disk_partitions(all=False):
                    try:
                        ...
                        info[f'mount_{part.device.replace("/","_")}'] = {
                            ...
                            'total_space': u.total, 'used_space': u.used,
                            'free_space': u.free, 'percent_used': u.percent,
                        }
                    except Exception:
                        pass
            except Exception:
...
def usage_route():
    usage = db.get_monitor_state('current_usage', {
        'disk_usage': 0, 'network_usage': 0,
        'disk_usage_percent': 0, 'network_usage_percent': 0,
        'max_disk_usage': 0, 'max_network_usage': 0,
        'below_threshold': False, 'idle_timer_active': False,
        'idle_time_remaining': 0, 'shutdown_scheduled': False,
        'last_updated': '',
    })
...
def reset_max_route():
    ...
    db.set_monitor_state('previous_values', {'disk_total': 0, 'network_total': 0, 'timestamp': 0})
    logger.warning(f"Máximos resetados. Anterior — disco: {old.get('max_disk_usage', 0):.0f} B/s, rede: {old.get('max_network_usage', 0):.0f} B/s")
    return jsonify({
        'status': 'success',
